...
class MassGatheringModel(BaseScipyClusterVacModel):
    ...
    universal_param_names = ['theta', 'kappa_D', 'epsilon_1', 'p_s',
                             'epsilon_2', 'epsilon_3', 'p_h', 'p_d',
                             'gamma_A_1', 'gamma_A_2',
                             'gamma_I_1', 'gamma_I_2',
                             'gamma_H']
    ...
    def ode(self, y, t, **arg_params):
        parameters = self.combine_parameters(arg_params)
        foi = self.foi(y, parameters['beta'], parameters['theta'], parameters['kappa_D'])
        ...
        for cluster in self.clusters:
            for vaccine_group in self.vaccine_groups:
                ...
                self.group_transfer(y, y_deltas, t, cluster, vaccine_group)
                ...
                y[states_index['G_I']] += prog_symptomatic_path - prog_LFD_sensitive_symptomatic_path
                y[states_index['G_A']] += prog_asymptomatic_path - prog_LFD_sensitive_asymptomatic_path
                y[states_index['P_I']] += prog_LFD_sensitive_symptomatic_path - prog_mid_infection_symptomatic_path
                y[states_index['P_A']] += prog_LFD_sensitive_asymptomatic_path - prog_mid_infection_asymptomatic_path
                y[states_index['M_A']] += prog_mid_infection_asymptomatic_path-prog_late_asymptomatic_stage
                y[states_index['M_I']] += prog_mid_infection_symptomatic_undetected-prog_late_symptomatic_stage
                y[states_index['M_D']] += prog_detected_in_community-prog_late_detected_stage
                y[states_index['H']] += prog_detected_at_hospital - hospital_recovery
                y[states_index['F_A']] += prog_late_asymptomatic_stage-asymptomatic_recovery
                y[states_index['F_I']] += prog_late_asymptomatic_stage-symptomatic_recovery
                y[states_index['F_D']] += prog_late_detected_stage-detected_recovery
